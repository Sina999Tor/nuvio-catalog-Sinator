...
async function tmdbFetchManyPreserveOrder(tmdbKey, kind, ids) {
  ...
  // POZN.: klíč má verzi "tmdbmeta2" (ne "tmdbmeta") schválně — starší
  // cache obsahovala id ve starém formátu "tmdb-movie-..." bez IMDb ID.
  // Změna verze klíče okamžitě zneplatní všechny staré záznamy (nemusíš
  // nic ručně mazat v Redisu) a při dalším dotazu se položka přenačte
  // s novým, "tt..." formátem ID, díky kterému ji najdou i stream doplňky.
  const cacheKeys = list.map((id) => `tmdbmeta2:${kind}:${id}`);
  ...
  for (let i = 0; i < missingIds.length; i += CONCURRENCY) {
    ...
    results.forEach((meta, j) => {
      ...
      if (meta) {
        out[pos] = meta;
        toCache.push([`tmdbmeta2:${kind}:${rawId}`, meta]);
